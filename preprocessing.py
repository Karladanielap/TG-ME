...
def image_crop(adata, save_path, library_id=None,crop_size='fit', target_size=224, verbose=False, ):
    ...
    tile_names = []
    with tqdm(total=len(adata), desc="Tiling image",
              bar_format="{l_bar}{bar} [ time left: {remaining} ]") as pbar:
        for imagerow, imagecol in zip(adata.obs["imagerow"], adata.obs["imagecol"]):
            maxi=max([adata.obs.Width.max(), adata.obs.Height.max()])
            imagerow_down = imagerow - maxi / 2
            imagerow_up = imagerow + maxi / 2
            imagecol_left = imagecol - maxi / 2
            imagecol_right = imagecol + maxi / 2
    ...
    def extract_image_feat(
        ...
        with tqdm(total=len(self.adata),
            ...
            for spot, slice_path in self.adata.obs['slices_path'].items():
                spot_slice = Image.open(slice_path).convert('RGB')
