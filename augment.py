...
def cal_cellcompo_weight(
  ct,data,
  cellcompo_dist_type = "cosine"
  ):
...
def cal_weight_matrix(
  ...
  if platform in ["Visium", "ST"]:
    ...
    unit = max(adata.obs['Width'].max(),adata.obs['Height'].max())
  ...
  print("The number of nearest tile neighbors in physical distance is: {}".format(physical_distance.sum()/adata.shape[0]))
  ...
  if platform in ["Visium", "ST", "slideseqv2", "stereoseq"]:
    ...
    cellcompo_correlation=  cal_cellcompo_weight(ct,data=adata.obs,cellcompo_dist_type = "cosine")
  else:
  ...
  print("Cell type composition correlation calculting Done!")
  if verbose:
    adata.obsm["gene_correlation"] = gene_correlation
    adata.obsm["physical_distance"] = physical_distance
    adata.obsm["cellcompo_correlation"] = cellcompo_correlation
...
def augment_gene_data(
  ...
  if isinstance(adata.X, np.ndarray):
    print(adata.X.shape, 'Augmenting...')
  ...
  #augement_gene_matrix=augement_gene_matrix*cell_type_comp
